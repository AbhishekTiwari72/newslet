	...
	for i in range(1, row_count_user):
		for t in range(0,featuresCount):
			if int(sheet2[i][0]) in userFeature:
				userFeature[int(sheet2[i][0])].append(math.sqrt(globalAvg/featuresCount))
			else: 
				userFeature[int(sheet2[i][0])] = [math.sqrt(globalAvg/featuresCount)]

	for key in docIdDict:
		for t in range(0,featuresCount):
			if key in itemFeature:
				itemFeature[key].append(math.sqrt(globalAvg/featuresCount))
			else: 
				itemFeature[key] = [math.sqrt(globalAvg/featuresCount)]
